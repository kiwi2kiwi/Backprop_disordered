...
#                        print("prevented perceptives connecting to same neuron")
    ...
    def spawn_neurons_axons(self, input_number, output_number):
        ...
        for o in self.output_set:
            Ns = self.find_x_nearest(o, self.hidden_set, connection_limit=20, x=8)
            for n in Ns:
                self.create_Axon(o, n)

        # axons generation from Input to 3 nearest neurons in processing neuron set
        for i in self.input_set:
            Ns = self.find_x_nearest(i, self.hidden_set, connection_limit=10, x=2)
            for n in Ns:
        ...
        for p in self.hidden_set:
            Ns = self.find_x_nearest(p, self.hidden_set, connection_limit=50, x=5)
            for n in Ns:
